...
Because the n-n coincidences comprising $Y_{\text{acc}}(\theta)$ consist of two independent detected neutrons, the same exact relationship exists among pairs of single neutrons that were detected during different pulses.
% given that the two different pulses occurred at virtually the same time and thus under the same experimental conditions.
Therefore, the opening angle distribution formed by pairing neutrons that were detected during different pulses, denoted $P_{\scaleto{DP}{4pt}}(\theta)$, is proportional to $Y_{\text{acc}}(\theta)$.
In other words,  $P_{\scaleto{DP}{4pt}}(\theta)$ and $Y_{\text{acc}}(\theta)$ have the same shape.
However, $Y_{\text{acc}}(\theta)$ is not equal to $P_{\scaleto{DP}{4pt}}(\theta)$, because there are, on average, twice as many detected neutrons in a pulse-pair than there are in a single pulse.
...
Let $\lambda$ represent the mean number of uncorrelated detected neutrons per pulse.
The accidental coincidence rate per pulse integrated over all opening angles, denoted by $\sum_{\theta} Y_{\text{acc}}(\theta)$, is equal to the probability of there being exactly two detected uncorrelated neutrons detected in a single pulse\footnote{Cases of greater than two-fold coincidence are not considered in this analysis, as it is not necessary because of the low detection rates during this work.}:% It can be shown, however, that accounting for any number of coincidences, from zero all the way up to $\infty$-fold coincident events in a pulse or pulse-pair, will give the same answer.}:
\begin{equation} \label{math:SP}
    ...
    \end{split}
\end{equation}
To determine the value of $\lambda$, one would need the ability to determine which events are correlated, as $\lambda$ quantifies the per-pulse rate of uncorrelated, independent detected neutrons.
Such information is not known, but the largest possible value for $\lambda$ is the mean number of detected neutrons per pulse, as this assumes that all neutrons are uncorrelated.
For this work, this places an upper bound on $\lambda^2$ of $3\times 10^{-5}$, which is small enough to neglect all terms on the order of $\lambda^3$ or greater.

For the case of different-pulse pairs, a n-n coincidence is said to occur when there is a detected neutron in both pulses.
In this case, the total per-pulse-pair n-n coincidence rate is the square of the probability of there being one detected neutron in a single pulse, since a pulse-pair n-n coincidence consists of two detected neutrons, one in each pulse.
Cases in which there are two or more detected neutrons in a given pulse can be neglected.
